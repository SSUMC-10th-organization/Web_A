  ...
  return (
    <div className="bg-black min-h-screen flex flex-col items-center pt-20 text-white">
      ...
      <div className="w-full max-w-sm flex items-center mb-10 relative">
        <button onClick={() => navigate(-1)} className="absolute left-0 text-2xl font-bold">
          {'<'}
        </button>
      ...
      <form className="w-full max-w-sm flex flex-col gap-6">
        ...
        <div className="flex flex-col">
          <input
            ...
            className={`w-full p-4 rounded-full bg-white text-black outline-none border-2 transition-colors ${
              errors.email && values.email.length > 0 ? 'border-red-500' : 'border-transparent'
            }`}
          />
          {/* 에러 메시지 (미션 2) */}
        ...
        <div className="flex flex-col">
          <input
            ...
            onChange={handleChange}
            className={`w-full p-4 rounded-full bg-white text-black outline-none border-2 transition-colors ${
              errors.password && values.password.length > 0 ? 'border-red-500' : 'border-transparent'
            }`}
          />
          {/* 에러 메시지 (미션 2) */}
        ...
        <button
          ...
          className={`w-full p-4 mt-4 rounded-full font-bold text-lg transition-colors ${
            isFormValid ? 'bg-[#FF0066] text-white cursor-pointer' : 'bg-gray-400 text-gray-700 cursor-not-allowed'
          }`}
        >
          로그인
        </button>
