...
const Layout = () => {
  ...
  const { isOpen: isSidebarOpen, close: closeSidebar, toggle: toggleSidebar } = useSidebar(false);
  ...
  return (
    <S.LayoutWrapper>
      ...
      <S.Body>
        {isSidebarOpen && <S.Overlay onClick={closeSidebar} />}
        ...
        >
          <S.MenuSection>
            <S.MenuItem onClick={() => { navigate('/search'); closeSidebar(); }}>
              🔍 찾기
            </S.MenuItem>
            <S.MenuItem onClick={() => { navigate('/mypage'); closeSidebar(); setIsEditOpen(true); }}>
              👤 마이페이지
            </S.MenuItem>
          </S.MenuSection>
