...
const CommentItem = ({ comment, onUpdate, onDelete }: Props) => {
  ...
  const nickname = comment.user_nickname || comment.nickname || comment.author?.nickname || '익명';
  const profile = comment.user_profile || comment.profileImage || comment.author?.profileImage;
  const content = comment.content || comment.text;

  const [editContent, setEditContent] = useState(content);

  const myNickname = authStorage.getUserInfo()?.nickname;
  const isMine = myNickname === nickname;
