...
const Pagination = ({ page, totalPages, onPageChange }: Props) => {
  ...
  return (
    <div className="flex items-center justify-center gap-1 py-8">
      ...
      {start > 1 && (
        <>
          <button onClick={() => onPageChange(1)} className="px-3 py-2 rounded-md text-sm text-white hover:bg-white/15 transition-colors cursor-pointer">1</button>
      ...
      {end < totalPages && (
        <>
          ...
          <button onClick={() => onPageChange(totalPages)} className="px-3 py-2 rounded-md text-sm text-white hover:bg-white/15 cursor-pointer transition-colors">{totalPages}</button>
        </>
      )}
